...
def root():
    ...
    with open('api/static/misc/getting_started.md') as f:
            contents = f.read()
    contents = Markup(markdown.markdown(contents, extensions=['tables', 'fenced_code']))
...
@app.route('/docs/gettingstarted', methods=['GET'])
def gettingstarted():
    contents = ""
    with open('api/static/misc/getting_started.md') as f:
            contents = f.read()
...
def docs_for(docName=""):
    api_documentation = {}
    name = ""
    if not os.path.exists('api/public/' + docName + '/info/info.json'):
        return render_template('404.html'), 404
    with open('api/public/' + docName + '/info/info.json') as f:
            data = json.load(f)
            name = data['name']
    contents = ""
    with open('api/public/' + docName + '/info/docs.md') as f:
            contents = f.read()
    contents = Markup(markdown.markdown(contents, extensions=['tables', 'fenced_code']))
